...
export default function WebRTCProvider({ children }: { children: React.ReactNode }) {
  ...
  const master_genworkerId = useSelector((state: any) => state.team.masterGenworker);
  const master_genworker = useSelector((state: any) => state.genworkersRepo.genworkers[master_genworkerId]);
  console.log("master_genworker:", master_genworker);
  const storage_genworkers = useSelector((state: any) => state.team.storageGenworkers);
  
  useEffect(() => {
    console.log("WebRTCProvider useEffect triggered", master_genworkerId, master_genworker);
    if (!socket) return;
    if (!master_genworker) return;
    const masterGenworkerSemId = `${master_genworker.ownerId}:${master_genworker.name}`;
    console.log("Setting up WebRTC connection with master_genworker:", masterGenworkerSemId);
    ...
    function initPeer(id) {
      ...
      peers.current[id].on("signal", (data) => {
        socket.emit("signal", { data, toGenworker: masterGenworkerSemId, from: userId });
      });
    ...
    socket.emit("get_signal", { genworkerId: masterGenworkerSemId, clientType: "USER" });
  ...
  const value = { 
    ...
    sendToMaster: (event, payload) => peers.current[`${master_genworker.ownerId}:${master_genworker.name}`].send(JSON.stringify({ event, payload, sender: userId }))
