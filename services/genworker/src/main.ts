...
import { GenWorkerModule } from '@genworker/genworker/genworker.module';
...
async function bootstrap() {
  const grpcApp = await NestFactory.createMicroservice<MicroserviceOptions>(GenWorkerModule, {
  ...
  const httpApp = await NestFactory.create(GenWorkerModule);
